...
- **Binance_Pay**: Binance's hosted merchant checkout service used as a second buyer payment option and as an optional seller withdrawal destination.
- **Binance_Order**: A persisted record of a Binance Pay order created on behalf of a deal, identified by `prepayId`.
- **Binance_Payout**: A Binance Pay payout request issued on admin approval of a `binance`-destination Withdrawal_Request, identified by `payoutTransactionId`.
...
### Requirement 21: Binance Pay Buyer Payment

**User Story:** As a buyer who prefers paying with crypto/USDT or with USD via Binance Pay, I want to pay a deal through Binance Pay's hosted checkout, so that I can fund the escrow without using a Cambodian bank or my BothSafe wallet balance.

#### Acceptance Criteria

1. WHILE Deal_Status is `READY_FOR_PAYMENT`, THE Payment service SHALL include `pay_with_binance` in `allowed_actions` for the buyer participant only.
2. WHEN the buyer triggers `pay_with_binance`, THE Payment service SHALL call Binance Pay's create-order API with merchant credentials and SHALL return `prepayId`, `qrcodeLink`, `deeplink`, `universalUrl`, `expireTime`, and the deal's `Reference_Note` within 5 seconds.
3. THE Payment service SHALL set the Binance Pay order Currency from the deal Currency: USD when the deal Currency is USD if the merchant tier supports FIAT receiving, otherwise USDT priced from the deal USD amount; KHR-denominated deals MUST be rejected with a `payment.binance_currency_unsupported` error.
4. WHEN the create-order request to Binance Pay succeeds, THE Payment service SHALL persist a `BinanceOrder` row with `prepayId UNIQUE`, status `PENDING`, deal id, deal amount, and an `expireTime` of at most 30 minutes from creation, and SHALL set Deal_Status to `PAYMENT_PENDING_VERIFICATION` in the same database transaction.
5. IF the create-order request to Binance Pay returns a non-success response or fails with a network error after 3 retries within 10 seconds, THEN THE Payment service SHALL leave Deal_Status at `READY_FOR_PAYMENT`, SHALL NOT persist a `BinanceOrder` row, and SHALL return a `payment.binance_unavailable` error to the buyer.
6. WHEN Binance Pay sends a webhook callback for an event in {`PAY_SUCCESS`, `PAY_CLOSED`, `PAY_REFUND`}, THE Payment service SHALL verify the request by recomputing the HMAC-SHA512 over `BinancePay-Timestamp + "\n" + BinancePay-Nonce + "\n" + raw_body + "\n"` using the merchant API secret AND by verifying the RSA signature on the body using the cached Binance public certificate identified by `BinancePay-Certificate-SN`.
7. IF a webhook callback fails either signature check defined in criterion 6, OR `BinancePay-Timestamp` is older than 5 minutes, OR the `merchantTradeNo` does not match a known `BinanceOrder`, THEN THE Payment service SHALL respond HTTP 401 with `code = "FAIL"`, SHALL NOT mutate any deal, ledger, or order row, and SHALL log the rejection with `prepayId`, certificate serial, and reason.
8. WHEN a verified webhook with event `PAY_SUCCESS` arrives for a `BinanceOrder` whose status is `PENDING`, THE Payment service SHALL within a single database transaction set `BinanceOrder.status = PAID`, write an `ESCROW_RECEIVED` Wallet ledger entry on the BothSafe escrow Wallet for the deal Currency, transition Deal_Status to `PAID_ESCROWED → SELLER_PREPARING`, write the corresponding Audit_Log entries, and enqueue `PAYMENT_VERIFIED` and `SELLER_SHOULD_SHIP` notification outbox rows.
9. WHEN a verified webhook with event `PAY_SUCCESS` arrives for a `BinanceOrder` whose status is already `PAID`, THE Payment service SHALL respond HTTP 200 with `code = "SUCCESS"` and SHALL NOT write any ledger entry, audit row, or status transition.
10. THE Payment service SHALL run a reconciliation processor that, every 60 seconds, selects every `BinanceOrder` in status `PENDING` that was created more than 60 seconds ago and calls Binance Pay's query-order endpoint; on a `PAID` response it SHALL settle the order using the same single-transaction logic defined in criterion 8, on an `EXPIRED` or `CANCELED` response it SHALL set `BinanceOrder.status` accordingly and revert Deal_Status from `PAYMENT_PENDING_VERIFICATION` to `READY_FOR_PAYMENT`.
11. THE Payment service SHALL store webhook idempotency keys keyed on `(prepayId, eventType, BinancePay-Nonce)` and SHALL reject duplicate keys with HTTP 200 and `code = "SUCCESS"` without writing any ledger entry, audit row, or status transition.
12. WHEN Binance Pay sends a webhook with event `PAY_REFUND` for a `BinanceOrder` whose status is `PAID`, THE Payment service SHALL set `BinanceOrder.status = REFUNDED`, write a `BUYER_REFUND_SENT` ledger entry on the buyer Wallet equal to the original deal amount, transition Deal_Status to `REFUNDED`, and enqueue a `REFUND_COMPLETED` notification, all within a single database transaction.
13. THE Payment service SHALL store the merchant API secret, the certificate cache, and any raw response bodies that contain payer identifiers only in fields that are redacted by the Pino logger configuration; raw merchant secret values SHALL never appear in audit log entries, error envelopes, or notification payloads.

### Requirement 22: Binance Pay Seller Withdrawal

**User Story:** As a seller who would rather receive payouts to my Binance account than via KHQR or a local bank, I want to choose Binance Pay as a withdrawal destination, so that BothSafe can transfer my withdrawal directly into my Binance balance.

#### Acceptance Criteria

1. THE Wallet_Service SHALL extend the Destination_Type set defined in Requirement 15 to {`khqr`, `bank`, `binance`} and SHALL accept a `Destination_Type` of `binance` on a Withdrawal_Request submitted by an authenticated seller.
2. WHERE Destination_Type is `binance`, THE Wallet_Service SHALL require a `Binance_Pay_Id` of 9 to 19 numeric digits OR a `Binance_Email` of 5 to 254 characters matching the email syntax used in Requirement 1; the request SHALL contain exactly one of the two fields.
3. WHERE Destination_Type is `binance`, THE Wallet_Service SHALL accept Currency only from the set {USD} for the MVP, and SHALL reject KHR-denominated `binance` withdrawal requests with a `withdrawal.invalid_field` error including a `currency` field detail; KHR support is deferred until the merchant tier confirms KHR fiat receiving.
4. WHEN a `binance` Withdrawal_Request passes validation, THE Wallet_Service SHALL place the same `SELLER_PAYOUT_PENDING` hold defined in Requirement 15.8 and SHALL set the Withdrawal_Request status to `pending_admin_review` in the same single database transaction.
5. WHEN an admin approves a `binance` Withdrawal_Request, THE Admin_Service SHALL call Binance Pay's payout API with the seller's `Binance_Pay_Id` or `Binance_Email`, the deal Currency, the requested Amount, and a unique `merchantSendId = withdrawal_request.id`; on a `PROCESSING` or `SUCCESS` response, the Admin_Service SHALL within a single database transaction write a `SELLER_PAYOUT_SENT` ledger entry on the seller Wallet, set Withdrawal_Request status to `paid`, persist the returned `payoutTransactionId` as `payout_reference`, and write an Audit_Log entry per Requirement 16.7.
6. IF the Binance Pay payout API returns a non-success response or fails with a network error after 3 retries within 10 seconds, THEN THE Admin_Service SHALL leave the Withdrawal_Request status at `pending_admin_review`, SHALL NOT write any ledger entry, and SHALL return a `withdrawal.binance_payout_failed` error including the Binance error code; the admin MAY retry the approve action.
7. WHEN Binance Pay sends a payout-callback webhook for a `payoutTransactionId` previously stored in `Withdrawal_Request.payout_reference`, THE Withdrawal service SHALL verify the signature using the same scheme defined in Requirement 21.6, and on a verified `SUCCESS` event SHALL be a no-op when the Withdrawal_Request status is already `paid`, on a verified `FAILED` event SHALL within a single database transaction write a compensating `ADJUSTMENT` credit equal to the held amount, set Withdrawal_Request status to `rejected` with reason `binance_payout_failed`, and write the Audit_Log entry.
8. THE Withdrawal service SHALL run a reconciliation processor that, every 60 seconds, selects every Withdrawal_Request in status `pending_admin_review` whose `payout_reference` is non-null and whose last status check was more than 60 seconds ago, and calls Binance Pay's query-payout endpoint; on a terminal `SUCCESS` or `FAILED` response it SHALL apply the same logic defined in criterion 7.
9. IF a non-admin User attempts to approve a `binance` Withdrawal_Request, THEN THE Admin_Service SHALL reject the request per Requirement 16.8 and SHALL NOT call the Binance Pay payout API.
